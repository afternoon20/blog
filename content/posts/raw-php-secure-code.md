...
`' OR '1'='1`のような入力で全件取得されたり、`'; DROP TABLE users; --`でテーブルを削除されるリスクがあります。
...
`../../etc/passwd`のような入力でサーバーの任意のファイルを読み取られるリスクがあります。
...
| 脆弱性 | 対策 |
| :--- | :--- |
| **XSS** | `htmlspecialchars`で出力時にエスケープ |
| **CSRF** | ワンタイムトークンの発行と`hash_equals`による検証 |
| **セッションハイジャック** | Cookie設定の厳格化と`session_regenerate_id`の使用 |
| **SQLインジェクション** | PDOプリペアドステートメントで値と命令を分離 |
| **OSコマンドインジェクション** | ホワイトリストからのみコマンドを選択 |
| **ディレクトリトラバーサル** | `basename`と`realpath`でパスを検証 |

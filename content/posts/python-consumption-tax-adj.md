...
description = 'Pythonで作成した弥生会計インポートクラスを拡張し、別行に分かれた消費税データを本体行に自動合算する方法を解説します。税率計算による正確な紐付けロジックを紹介。'
tags = ['Python', '弥生会計']
+++

[Pythonで弥生会計のインポートファイルを作成する記事](/posts/yayoi-to-csv/)の続編です。
会計ソフトや販売管理システムから出力したデータでは、本体価格と消費税が別々の行に分かれていることが多々あります。
これらを弥生会計にインポートする際、1行の「税込仕訳（内税形式）」にまとめたい場合があります。
...
実際のコードは以下の通りです。
科目名（仮払消費税など）をトリガーに、同じ伝票内で金額比（10%または8%）が一致する行を探して合算するコードです。

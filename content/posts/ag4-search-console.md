...
Hugoでブログを公開したら、最初に行うべきGA4（Googleアナリティクス）とGoogle Search Consoleの導入手順を解説します。 
...
PaperModテーマはデフォルトで本番環境（production）のみGA4を読み込む仕様になっています。
`hugo.toml`に以下の設定が入っていないとローカル・本番どちらでもGA4が動作しません。
...
設定後、リアルタイムレポートでアクセスが計測されているか確認してください。

### 運用時の注意点

設定直後は「リアルタイムレポート」で自分のアクセスがカウントされているか確認します。
確認後は正確なデータを取るために「内部トラフィックの除外」設定を行います。

**内部トラフィック除外の手順**

1. Googleアナリティクス > 管理 > データストリームを開く
2. 対象のストリームを選択 > タグ設定を行う > 内部トラフィックの定義をクリック
3. 「作成」をクリックしてルール名を入力（例：自宅IP）
4. IPアドレスの値に自分のグローバルIPを入力して保存
5. 管理 > データフィルタ に戻り、作成したフィルタを「有効」に変更

グローバルIPは[https://whatismyipaddress.com/](https://whatismyipaddress.com/)などで確認できます。
...
所有権の確認が完了したら、サイトマップを送信してGoogleにサイト構造を伝えます。

1. Search Consoleの左メニューから「サイトマップ」を選択
2. 「新しいサイトマップの追加」にサイトマップのURLを入力
   - Hugoの場合：`https://yourdomain.com/sitemap.xml`
3. 「送信」をクリック

送信後、ステータスが「成功しました」になればOKです。
反映には数日〜数週間かかることがあります。

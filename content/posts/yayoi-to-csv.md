...
## はじめに
Pythonを活用して、弥生会計のインポートファイル（CSV）作成の自動化を図りたいと思います。 
本記事では汎用的なインポートクラスを作成し、これを元にExcelからインポートファイルの作成を実現します。 
今回は、実務でよくあるExcelにまとめられた取引データを読み込み、弥生会計が正しく認識できるインポート用CSVを出力するまでの一連の流れを解説します。
...
## 弥生クラスの定義
まずはインポートデータをプロパティにもつクラスを作成します。弥生会計の25項目を順番に並べたリストを返すメソッドも実装しています。
特に設定が不要なものや必須項目はいくつかデフォルト値を設定しています。
